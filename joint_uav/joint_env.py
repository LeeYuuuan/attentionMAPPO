...
class AttentionChargingEnv:
    ...
    def _apply_original_charging_transitions(self, actions: np.ndarray) -> np.ndarray:
        ...
        for i, u in enumerate(self.uavs):
            # Interpret each action exactly once in the status in which it was
            # selected.  In particular, CHARGING+0 means "keep charging"; if
            # that UAV is auto-released because it became full, the same 0
            # must not be reinterpreted as a fresh SERVING charge request.
            if prev_status[i] != SERVING or u.status != SERVING or not want_charge[i]:
                continue
            if free > 0:
