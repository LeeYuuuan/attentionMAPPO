...
class SharedCategoricalActor(nn.Module):
    def __init__(self, obs_dim: int, hidden: int = 512) -> None:
        super().__init__()
...
class MAPPOConfig:
    ...
    epochs: int = 10
    actor_minibatch_size: int = 2048
    critic_minibatch_size: int = 2048
    max_grad_norm_actor: float = 0.5
    max_grad_norm_critic: float = 2.0
...
class MAPPO:
    ...
    def update(
        ...
        for _ in range(self.cfg.epochs):
            ...
            actor_batches = [
                actor_idx[i:i + self.cfg.actor_minibatch_size]
                for i in range(0, len(actor_idx), self.cfg.actor_minibatch_size)
            ]
            critic_batches = [
                critic_idx[i:i + self.cfg.critic_minibatch_size]
                for i in range(0, len(critic_idx), self.cfg.critic_minibatch_size)
            ]
            for mb in actor_batches:
                ...
                nn.utils.clip_grad_norm_(
                    self.actor.parameters(), self.cfg.max_grad_norm_actor
                )
                self.actor_opt.step()
                with torch.no_grad():
            ...
            for mb in critic_batches:
                if mb.size == 0:
                    continue
                j = torch.as_tensor(mb, dtype=torch.long, device=self.device)
                v = self.critic(critic_obs[j])
                if self.cfg.value_clipping:
                    v_clip = old_values_n[j] + (v - old_values_n[j]).clamp(
                        -self.cfg.value_clip_range, self.cfg.value_clip_range
                    )
                    loss_unclip = F.mse_loss(v, returns_n[j], reduction="none")
                    loss_clip = F.mse_loss(v_clip, returns_n[j], reduction="none")
                    critic_loss = torch.maximum(loss_unclip, loss_clip).mean()
                else:
                    critic_loss = F.mse_loss(v, returns_n[j])
                self.critic_opt.zero_grad(set_to_none=True)
                (self.cfg.value_coef * critic_loss).backward()
                nn.utils.clip_grad_norm_(
                    self.critic.parameters(), self.cfg.max_grad_norm_critic
                )
