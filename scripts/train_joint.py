...
def save_eval_plots(
    ...
    *, save_block_grids: bool = True,
    ...
    # Battery includes reset state at index 0.  Status schedule describes the
    # result of each upper action, so it intentionally starts at index 1.
    battery = np.asarray(env.history["battery"], np.float32)
    status_all = np.asarray(env.history["status"], np.int8)
    status = status_all[1:]
    ...
    for i in range(env.n_uav):
        ax.plot(
            np.arange(len(battery)), battery[:, i], label=f"UAV{i}",
            linewidth=1.4, marker="o", markersize=2.5,
        )
        dead = np.flatnonzero(status_all[:, i] == 3)
        if dead.size:
            d = int(dead[0])
            ax.scatter(d, battery[d, i], marker="x", s=45, color="black", zorder=20)
    ax.set(title="Battery Traces (x=0 is reset)",
           xlabel="Upper-level state index (0=reset)", ylabel="Battery level")
...
class TrainingRolloutCollector:
    ...
    def collect(
        ...
        while steps < target_steps and (
            max_completed_episodes is None
            or self.completed_episodes < max_completed_episodes
        ):
            if self.obs is None:
                self._start_episode()
...
def main() -> None:
    ...
    while collector.completed_episodes < total_episodes:
        ...
        remaining_episodes = max(0, total_episodes - collector.completed_episodes)
        eta_seconds = (
